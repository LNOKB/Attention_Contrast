library(tidyverse)
data <- read_csv("dat_detect_reshaped.csv")

summary_stats <- data %>%
  group_by(participant, contrast, cue_type) %>%
  summarise(
...
p <- ggplot(summary_stats, aes(x = contrast, y = mean_detectrate,
                               color = factor(cue_type), group = factor(cue_type))) +
